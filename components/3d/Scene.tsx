...
export default function Scene() {
    ...
    if (crashed) {
        return (
            <div className="w-full h-full absolute inset-0 z-10 flex items-center justify-center bg-black overflow-hidden">
                <Image 
                    src="/models/preview-hero.png"
                    alt="3D Preview" 
                    fill 
                    className="object-cover opacity-90"
                    priority
                />
                <div className="absolute bottom-6 right-6 text-[10px] font-mono text-red-500/50 bg-black/20 px-2 py-1 rounded backdrop-blur-sm">
                    [ LOW POWER MODE ]
                </div>
            </div>
        )
