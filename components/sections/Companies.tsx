...
export function Companies() {
  return (
    <section className="bg-zinc-950 py-24 border-t-4 border-brand-white/10 relative z-20">
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {stats.map((stat, index) => (
            <div 
              key={index} 
              className="bg-zinc-900 border-4 border-zinc-800 p-8 md:p-12 hover:border-brand-red hover:-translate-y-2 transition-all duration-300 group"
            >
              <h3 className="font-display text-6xl md:text-8xl font-bold text-white mb-4 group-hover:text-brand-red transition-colors">
                {stat.value}
              </h3>
              <p className="font-mono text-zinc-400 uppercase tracking-widest text-sm md:text-base border-t border-zinc-700 pt-4 group-hover:border-brand-red/50 group-hover:text-white transition-colors">
                {stat.label}
              </p>
            </div>
          ))}
