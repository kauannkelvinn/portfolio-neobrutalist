...
export function Skills() {
  return (
    <section id="skills" className="bg-zinc-950 text-white py-32 border-t-4 border-white/10 relative z-20">
            ...
            <div className="grid grid-cols-2 gap-x-12 gap-y-16">
              {skillCategories.map((category, index) => (
                <FadeIn key={category.title} delay={index * 0.1}>
                    <div>
                    <h3 className="font-heading text-brand-red mb-6 uppercase tracking-[3] text-sm border-b border-brand-red/20 pb-2 inline-block">
                        {category.title}
                    </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {certificates.map((cert, index) => (
                <FadeIn key={`cert-${index}`} delay={0.3 + (index * 0.1)}>
                    <div
                    className={`relative group aspect-video border-2 border-zinc-800 bg-zinc-900 overflow-hidden cursor-pointer ${cert.status === 'locked' ? 'opacity-50 grayscale' : 'hover:border-brand-red'
                        }`}
                    >
